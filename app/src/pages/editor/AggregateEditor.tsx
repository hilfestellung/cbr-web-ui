...
import { PlusSquare, TrashFill, Trash2Fill } from "react-bootstrap-icons";
import { useSelector } from "react-redux";

import { ClassesSelector } from "../../modules/classes";

function AggregateEditor({ aggregate }: any) {
  const { t } = useTranslation();

  const classes = useSelector(ClassesSelector.getItems);

  const [editableAggregate, setEditableAggregate] = useState<any>(aggregate);
  const [newAttributeId, setNewAttributeId] = useState<any>("");
  const [newAttributeType, setNewAttributeType] = useState<any>("");
    ...
    return (
      <div>
        <Form>
          ...
          {editableAggregate.attributes &&
            editableAggregate.attributes.map((attribute: any) => (
